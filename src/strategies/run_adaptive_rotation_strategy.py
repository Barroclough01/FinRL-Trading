...
def run_single_date(
    ...
    print(
        f"   - Data range: {min(s.index.min() for s in price_data.values())} to {as_of_date}"
    )

    # 3. Run strategy
    print(f"\n3. Running strategy to generate target weights...")
    weights, audit_log = engine.run(price_data=price_data, as_of_date=as_of_date)
    ...
    for symbol, weight in sorted(
        weights.weights.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    # 6. Save structured JSON output if requested
    if json_output_path:
        import json

        ranked_groups = audit_log.group_strength.get("ranked_groups", [])
        fallback_status = audit_log.portfolio.get("constraints", {}).get(
        ...
        output_data = {
            ...
            "regime_state": weights.regime_state,
            "active_groups": weights.active_groups,
            "ranked_groups": ranked_groups,
            "fallback_status": fallback_status,
            "audit_file_path": str(audit_file.resolve()),
        }
        ...
        print(f"\nStructured JSON output saved to: {out_path}")
...
def run_backtest(
    ...
    for i, date in enumerate(decision_dates, 1):
        try:
            ...
            # Priority 2: Fast Risk-Off (only if not weekly)
                ...
                from src.strategies.adaptive_rotation.market_regime import (
                    check_fast_risk_off_trigger,
                )
                ...
                if should_trigger_fast and (
                    fast_track_active_until is None or date > fast_track_active_until
                ):
                    decision_type = "fast_track_trigger"
                    fast_track_active_until = date + pd.Timedelta(
                        days=config.fast_risk_off.behavior.duration_days
                    )
                    trigger_reasons.append(
                        f"fast_risk_off(Price={signals['price_shock']},Vol={signals['volatility_shock']})"
                    )

                    print(
                        f"   [FAST TRACK TRIGGERED] {date.strftime('%Y-%m-%d')}: "
            ...
            if (
                decision_type != "weekly"
                and decision_type != "fast_track_trigger"
                and current_positions
            ):
                # Get current prices for all positions
                current_prices = {}
                for symbol in current_positions.keys():
                    daily_symbol_data = preprocessor.get_daily_data_as_of(
                        date, symbols=[symbol]
                    )
                ...
                risk_result = engine.risk_manager.check_stops(
                    current_positions, current_prices, date
                )

                if len(risk_result.triggered_stops) > 0:
                    decision_type = "stop_loss_trigger"
                    trigger_reasons.append(
                        f"stop_loss({len(risk_result.triggered_stops)}_positions)"
                    )

                    stopped_symbols = [s.symbol for s in risk_result.triggered_stops]
                    print(
                        f"   [STOP-LOSS TRIGGERED] {date.strftime('%Y-%m-%d')}: {', '.join(stopped_symbols)}"
                    )
            ...
            if decision_type == "weekly":
                ...
                for symbol, weight in weights.weights.items():
                    if weight > 0 and symbol in price_data:
                        ...
                        from src.strategies.adaptive_rotation.risk_manager import (
                            PositionState,
                        )
                ...
                for symbol in list(current_positions.keys()):
                    ...
                    if (
                        symbol in daily_symbol_data
                        and len(daily_symbol_data[symbol]) > 0
                    ):
                ...
                print(
                    f"   [FAST TRACK ADJUSTMENT] Would reduce exposure to {target_invested:.1%} (event logged)"
                )
                ...
                print(
                    f"   [STOP-LOSS ADJUSTMENT] Exited {len(stopped_symbols)} positions: "
                    f"{', '.join(stopped_symbols)}"
                )
                print(
                    f"      Cooldown activated: {cooldown_days} days (blocks re-entry)"
                )
                    ...
                    for symbol in list(current_positions.keys()):
                        ...
                        if symbol in daily_symbol_data:
                            ...
                            if len(symbol_prices) > 0:
                                ...
                                current_positions[symbol].update_peak(
                                    current_price, date
                                )

            # Progress indicator
            if i % 100 == 0 or (
                decision_type in ["fast_track_trigger", "stop_loss_trigger"]
            ):
                print(
                    f"   Progress: {i}/{len(decision_dates)} days scanned, "
    ...
    if daily_fast_track:
        ...
        print(
            f"  Fast Track adjustments: {fast_track_adjustment_count} (Reduce exposure)"
        )
    ...
    if results:
        ...
        weights_detail_file = (
            weights_dir / f"ars_portfolio_weights_{start_date}_to_{end_date}.csv"
        )
        ...
        _generate_performance_report(
            ...
            start_date,
            end_date,
            data_dir or str(config.paths.data_root),
            weights_dir,
        )
...
def _generate_performance_report(
    weights_df, start_date, end_date, data_dir, output_dir
):
    ...
    equity = pd.DataFrame(
        {"date": pd.to_datetime(dates), "portfolio": portfolio_values}
    ).set_index("date")
    ...
    try:
        ...
        fig.text(
            ...
            fontsize=11,
            bbox=dict(
                boxstyle="round,pad=0.4", facecolor="#f0f9ff", edgecolor="#bfdbfe"
            ),
        )
...
Examples:
  ...
  python run_adaptive_rotation_strategy.py \\
    ...
    # Single run mode
    parser.add_argument(
        "--date", type=str, help="Run for single date (format: YYYY-MM-DD)"
    )
    ...
    parser.add_argument(
        "--start", type=str, help="Backtest start date (format: YYYY-MM-DD)"
    )

    parser.add_argument(
        "--end", type=str, help="Backtest end date (format: YYYY-MM-DD)"
    )
