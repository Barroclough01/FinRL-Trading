...
class AdaptiveRotationEngine:
    ...
    def run(
        ...
        audit_data = {
            "data_summary": {
                ...
                "date_range": (
                    min(
                        s.index[0] for s in prices_as_of.values() if len(s) > 0
                    ).strftime("%Y-%m-%d"),
                    max(
                        s.index[-1] for s in prices_as_of.values() if len(s) > 0
                    ).strftime("%Y-%m-%d"),
                ),
        ...
        group_rankings = self._rank_assets_in_groups(
            prices_as_of, group_strength.active_groups, as_of_date
        )
        ...
        # Create audit log
        audit_log = AuditLog(date=as_of_date, **audit_data)
    ...
    def _detect_regime(self, prices: Dict[str, pd.Series], as_of_date: pd.Timestamp):
        ...
        if self.data_preprocessor is not None:
            try:
                daily_data = self.data_preprocessor.get_daily_data_as_of(
                    as_of_date=as_of_date, symbols=["^GSPC", "^VIX", "QQQ"]
                )
    ...
    def _analyze_group_strength(
        self, prices: Dict[str, pd.Series], as_of_date: pd.Timestamp
    ):
        ...
        for group_name, ranking in group_rankings.items():
            for symbol, score in ranking.asset_scores.items():
                if score.is_valid:
                    ...
                    all_zscores[symbol] = pd.Series([score.zscore], index=[as_of_date])
    ...
    def _check_stops(
        ...
        return self.risk_manager.check_stops(positions, current_prices, as_of_date)
    ...
    def _audit_exceptions(self, exceptions) -> Dict:
        ...
        return {
            "persistence_rule_status": exceptions.persistence_rule_status,
            "strong_signal_rule_status": exceptions.strong_signal_rule_status,
    ...
    def export_weights_to_dataframe(
        results: List[Dict], all_symbols: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
...
if __name__ == "__main__":
    ...
    for sym in ["^GSPC", "^VIX", "QQQ"]:
        prices[sym] = pd.Series(
            100 + pd.Series(range(len(dates))).values * 0.5, index=dates
        )

    # Sample assets
    for sym in ["AAPL", "MSFT", "NVDA", "XOM", "CVX"]:
        prices[sym] = pd.Series(
            100 + pd.Series(range(len(dates))).values * 0.3, index=dates
        )
    ...
    try:
        weights, audit = engine.run(price_data=prices, as_of_date=test_date)
